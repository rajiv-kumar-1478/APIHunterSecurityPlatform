...
public class CohereProviderAdapter : IAiProvider
{
    ...
    public CohereProviderAdapter(HttpClient httpClient, IDataProtectionProvider protectionProvider, AiProviderConfig config)
    {
    ...
    public async Task<AiPromptResponse> CompletePromptAsync(AiPromptRequest request, CancellationToken ct = default)
    {
        ...
        using var httpRequest = new HttpRequestMessage(HttpMethod.Post, "https://api.cohere.com/v2/chat")
        {
        ...
        try
        {
            ...
            if ((httpResponse.Headers.TryGetValues("x-ratelimit-remaining", out var remValues) ||
                 httpResponse.Headers.TryGetValues("x-ratelimit-remaining-requests", out remValues)) &&
            ...
            if (!httpResponse.IsSuccessStatusCode)
            {
                ...
                return new AiPromptResponse(
                    ...
                    LatencyMs: stopwatch.ElapsedMilliseconds,
                    ErrorCode: errorCode,
                    ErrorMessage: $"Cohere returned status code {(int)httpResponse.StatusCode} ({httpResponse.StatusCode}).",
            ...
            if (root.TryGetProperty("message", out var msgElement) &&
                msgElement.TryGetProperty("content", out var contentArray) &&
            ...
            else if (root.TryGetProperty("text", out var rootTextProp))
            {
                contentText = rootTextProp.GetString() ?? string.Empty;
            }
            ...
            {
                if (usage.TryGetProperty("tokens", out var tokens))
                {
                    if (tokens.TryGetProperty("input_tokens", out var pt)) promptTokens = pt.GetInt32();
                    if (tokens.TryGetProperty("output_tokens", out var ctProp)) completionTokens = ctProp.GetInt32();
                }
                else if (usage.TryGetProperty("billed_units", out var billed))
                {
                    if (billed.TryGetProperty("input_tokens", out var pt)) promptTokens = pt.GetInt32();
                    if (billed.TryGetProperty("output_tokens", out var ctProp)) completionTokens = ctProp.GetInt32();
                }
            }
            else if (root.TryGetProperty("meta", out var meta))
            {
                if (meta.TryGetProperty("billed_units", out var billed))
                {
                    if (billed.TryGetProperty("input_tokens", out var pt)) promptTokens = pt.GetInt32();
                    if (billed.TryGetProperty("output_tokens", out var ctProp)) completionTokens = ctProp.GetInt32();
                }
                else if (meta.TryGetProperty("tokens", out var tokens))
                {
                    if (tokens.TryGetProperty("input_tokens", out var pt)) promptTokens = pt.GetInt32();
                    if (tokens.TryGetProperty("output_tokens", out var ctProp)) completionTokens = ctProp.GetInt32();
                }
        ...
        catch (OperationCanceledException) when (ct.IsCancellationRequested)
        {
            ...
            return new AiPromptResponse(
                ...
                LatencyMs: stopwatch.ElapsedMilliseconds,
                ErrorCode: "Timeout",
                ErrorMessage: "Cohere request timed out or was cancelled.",
                IsRetryable: true);
        }
        catch (Exception)
        {
